...
## Agno vs Vercel AI SDK: Key Mapping

| Vercel AI SDK | Agno |
|---------------|------|
| `generateText()` | `Agent.run()` |
| `tool({ parameters, execute })` | Python function with type hints + docstring |
| `maxSteps: 5` | `tool_call_limit=5` |
| Tool result as object | Tool result as string/serializable value |
| Built-in streaming (`useChat`) | `stream=True` + `stream_events=True` |
| Auto message history | `add_history_to_context=True` |
| Implicit tool routing | Automatic in Agent loop |
| `ai` package | `agno` package (Python) |
| TypeScript-first | Python-first |

## Agno vs LangChain: Key Mapping

| LangChain (createReactAgent) | Agno |
|------------------------------|------|
| `createReactAgent({ llm, tools })` | `Agent(model=model, tools=[func])` |
| `StateGraph` with nodes/edges | Implicit agent loop (Agent handles the loop) |
| `maxIterations: 5` | `tool_call_limit=5` |
| `MemorySaver` / `SqliteSaver` | `db=SqliteDb()` (built-in persistence layer) |
| `invoke()` | `run()` / `arun()` |
| `stream()` | `Agent(stream=True)` |
| Explicit message management | Agno builds context automatically |
| `thread_id` in configurable | `session_id` in agent.run() |
| `class extends Tool` | Python function with type hints |

## Agno vs CrewAI: Key Mapping

| CrewAI | Agno |
|--------|------|
| `Crew.kickoff()` | `Agent.run()` |
| `new Agent({ role, goal, tools })` | `Agent(model=model, tools=[func], instructions=[...])` |
| `new Task({ description, agent })` | `agent.run(user=prompt)` (prompt is inline) |
| `process: 'sequential'` | Single-agent paradigm (Team for multi-agent) |
| `maxIter: 5` | `tool_call_limit=5` |
| Tool as class | Tool as Python function |
| 3-layer: Agent → Task → Crew | 1-layer: Agent (or Team/Workflow primitives) |

## Agno vs LangGraph: Key Mapping

| LangGraph | Agno |
|-----------|------|
| `StateGraph().addNode().compile()` | `Agent()` — no explicit graph needed |
| `maxIter: 5` | `tool_call_limit=5` |
| `MemorySaver` | `db=SqliteDb()` / built-in sessions |
| `invoke()` | `run()` / `arun()` |
| `stream()` with `StreamMode` | `stream=True` + `stream_events=True` |
| Explicit nodes/edges | Implicit agent loop |
| `Annotation.Root` state | `session_state` dict |
| Composable sub-graphs | `Team` and `Workflow` primitives |
