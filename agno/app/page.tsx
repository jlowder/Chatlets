  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (data.error) {
        setMessages(prev => [
          ...prev,
          { role: 'assistant', text: `Error: ${data.error}`, toolOutputs: [] },
        ]);
      } else {
        setMessages(prev => [
          ...prev,
      ...
      setMessages(prev => [
        ...prev,
        { role: 'assistant', text: 'Could not connect to agent service. Make sure it is running on port 8081.', toolOutputs: [] },
      ]);
    } finally {
      setLoading(false);
    }
